...
def main():
    ...
    parser.add_argument(
        "--imgs-to-evaluate", default=500, type=int, help="number of images to evaluate"
    )
    parser.add_argument(
        "--weights-file",
    ...
    for idx in range(args.imgs_to_evaluate):
        ...
        with torch.no_grad():
            boxes, labels, scores, image_process_time = model.predict(image, device.type)
        if device.type == "cuda":
            torch.cuda.synchronize()
        detect_time = time.time() - detect_time_begin - image_process_time
        print('{:3d}/{:3d} inference time : {:.6f}\r'.format(idx, args.imgs_to_evaluate, detect_time), end='')
