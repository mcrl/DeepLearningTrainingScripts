...
class SSD(nn.Module):
    ...
    def predict(self, images, device_type):
        """
        Arguments:
        ...
        image_process_time_begin = time.time()
        images = self.preprocess(images)
        if device_type == "cuda":
            torch.cuda.synchronize()
        image_process_time = time.time() - image_process_time_begin
        scores, boxes = self(images)
        boxes, labels, scores = self.filter_results(scores, boxes)
        boxes = self.rescale_boxes(boxes, height, width)
        return boxes, labels, scores, image_process_time
