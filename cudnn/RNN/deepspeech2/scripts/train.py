...
parser.add_argument('--batch-size-test', default=8, type=int, help='Batch size for test')
...
parser.add_argument('--dist-url', default='tcp://192.168.0.100:15531', type=str,
...
if __name__ == '__main__':
    ...
    if not args.distributed:
        ...
        test_sampler = BucketingSampler(test_dataset, batch_size=args.batch_size_test)
    else:
        train_sampler = DistributedBucketingSampler(train_dataset, batch_size=args.batch_size,
                                                    num_replicas=args.world_size, rank=args.rank)
        test_sampler = DistributedBucketingSampler(test_dataset, batch_size=args.batch_size_test,
                                                    num_replicas=args.world_size, rank=args.rank)
    ...
    '''
    # modify the file name to what you want
    with open("bin/librispeech_train_wav.bin", "wb") as f_wav, open("bin/librispeech_train_txt.bin", "wb") as f_txt:
        print(len(train_dataset))

        i = 0

        f_wav.write(struct.pack('<i', len(train_dataset)))
        f_txt.write(struct.pack('<i', len(train_dataset)))

        for sample in train_dataset:
            i = i + 1
            if i % 1000 == 0:
                print(str(i) + "/" + str(len(train_dataset)))

            wav_length = len(sample[0][0])
            txt_length = len(sample[1])

            f_wav.write(struct.pack('<i', wav_length))
            f_wav.write(bytes(array('f',sample[0].numpy().flatten())))

            f_txt.write(struct.pack('<i', txt_length)) 
            f_txt.write(bytes(array('i', sample[1])))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print("rank : %d, total_time : %d" % (args.rank, total_time))
    ...
    print("End! total_time : %d" % (total_time))
